...
def test_load_config_workspace_registry_custom_db_path():
    env = dict(REQUIRED)
...
def test_load_config_workspace_registry_can_be_explicitly_disabled():
    env = dict(REQUIRED)
    env["KASM_ENABLE_WORKSPACE_REGISTRY"] = "false"
    cfg = load_config(env)
